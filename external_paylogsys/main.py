...
def respond():
    try:
        ...
        event = WebhookEvent(data=json.dumps(data), ip_address=ip_address)  # Include IP address when creating event
...
def get_logs():
    try:
        ...
        return jsonify(logs=[{"id": event.id, "timestamp": event.timestamp, "ip_address": event.ip_address, "data": json.loads(event.data)} for event in events])
    except Exception as e:
        print(e)
        return jsonify(status='ERROR', message=str(e)), 500
